...
# Check input
if (!exists("data") || !is.survey(data)) {
  stop("This template is meant to be used with a Survey.", call. = FALSE)
} else {
  srv <- data$clone(deep = TRUE)
}

# Get entity name and case-insensitive latent names ---------------------------
mainentity <- unname(get_association(srv, "mainentity"))
...
- Svarene er avgitt på web (Research.net[^1]) i perioden xx til yy.
